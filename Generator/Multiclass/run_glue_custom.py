...
from compute_metrics import glue_compute_metrics as compute_metrics
from glue_custom import glue_output_modes as output_modes
from glue_custom import glue_processors as processors
from glue_custom import glue_convert_examples_to_features as convert_examples_to_features
...
def evaluate(args, model, tokenizer, prefix="",evaluate=True,test=False):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        eval_dataset = load_and_cache_examples(args, eval_task, tokenizer, evaluate=evaluate,test=test)
...
def main():
    ...
    parser.add_argument("--label_dir", default=None, type=str,
                        help="The label directory where the output spaces are.")
    ...
    parser.add_argument("--do_train_pred", action='store_true',
    ...
    if args.task_name in ['ask', 'twa','smm4h','n2c2']:
        label_list = processor.get_labels(args.label_dir)
    else:
        label_list = processor.get_labels()
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model.to(args.device)
            result, predictions = evaluate(args, model, tokenizer, prefix=prefix,evaluate=True,test=False)
    ...
    if args.do_test and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model = model_class.from_pretrained(checkpoint)
            model.to(args.device)
            result, predictions = evaluate(args, model, tokenizer, prefix=prefix, evaluate=False, test = True)
            result = dict((k + '_{}'.format(global_step), v) for k, v in result.items())
            results.update(result)
    ...
    if args.do_train_pred and args.local_rank in [-1, 0]:
        if not args.do_train:
            output_dir = args.model_name_or_path
        else:
            output_dir = args.output_dir
        ...
        for checkpoint in checkpoints:
            ...
            model = model_class.from_pretrained(checkpoint)
            model.to(args.device)
            result, predictions = evaluate(args, model, tokenizer, prefix=prefix, evaluate=False, test = False)
            result = dict((k + '_{}'.format(global_step), v) for k, v in result.items())
            results.update(result)
